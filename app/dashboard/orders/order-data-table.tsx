...
export function OrderDataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
  ...
  return (
    <div className="border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000]">
      {/* Header */}
      <div className="bg-purple-700 border-b-2 border-black px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <p className="text-base font-black text-white uppercase tracking-wide">Your Orders</p>
          <p className="text-xs text-purple-200 mt-0.5">Review your ticket orders</p>
        </div>
        <input
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-b-2 border-black bg-purple-50 hover:bg-purple-50">
                {headerGroup.headers.map((header) => (
                  <TableHead key={header.id} className="text-xs font-black uppercase tracking-wide text-black border-r border-black/20 last:border-r-0">
